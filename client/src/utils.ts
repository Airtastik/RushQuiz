...
function questionToCsvRow(q: Question): CsvRow {
  const base: CsvRow = { id: q.id, type: q.type, text: q.text, points: q.points, timeLimit: q.timeLimit };
  if (q.type === "multiple_choice") {
    const mc = q as MultipleChoiceQuestion;
    return { ...base, option_a: mc.options[0], option_b: mc.options[1], option_c: mc.options[2], option_d: mc.options[3], correctIndex: mc.correctIndex };
  }
  if (q.type === "short_answer") {
    const sa = q as ShortAnswerQuestion;
    return { ...base, correctAnswer: sa.correctAnswer, acceptedAnswers: sa.acceptedAnswers.join("|") };
  }
  if (q.type === "numeric") {
    const nq = q as NumericQuestion;
    return { ...base, correctAnswer: String(nq.correctAnswer), tolerance: nq.tolerance, unit: nq.unit ?? "" };
  }
  return base;
}
...
export function downloadCsv(quiz: Quiz): void {
  ...
  a.href = url;
  a.download = `${quiz.title.replace(/\s+/g, "_")}_blitztrivia.csv`;
...
export function csvToQuiz(csvText: string): Quiz | null {
  try {
    ...
    for (let i = 1; i < dataLines.length; i++) {
      ...
      const base = { id: row.id as string || uid(), text: row.text as string || "", points: Number(row.points) || 100, timeLimit: Number(row.timeLimit) || 15 };
      if (type === "multiple_choice") {
        questions.push({ ...base, type: "multiple_choice" as const, options: [row.option_a as string||"", row.option_b as string||"", row.option_c as string||"", row.option_d as string||""], correctIndex: Number(row.correctIndex) || 0 });
      } else if (type === "short_answer") {
        questions.push({ ...base, type: "short_answer" as const, correctAnswer: row.correctAnswer as string || "", acceptedAnswers: row.acceptedAnswers ? String(row.acceptedAnswers).split("|").filter(Boolean) : [] });
      } else if (type === "numeric") {
        questions.push({ ...base, type: "numeric" as const, correctAnswer: Number(row.correctAnswer) || 0, tolerance: Number(row.tolerance) || 0, unit: row.unit as string || undefined });
      }
    }
