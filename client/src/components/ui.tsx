...
import { C, FONT_BODY, FONT_DISPLAY, APP_NAME } from "../constants";
...
export const Btn: React.FC<BtnProps> = ({ children, onClick, variant = "primary", size = "md", disabled, style = {}, type = "button", title }) => {
  ...
  return (
    <button type={type} title={title} disabled={disabled} onClick={disabled ? undefined : onClick} style={{
      fontFamily: FONT_BODY, fontWeight: 700, letterSpacing: "0.07em",
      textTransform: "uppercase" as const, border: "none", borderRadius: 5,
      cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.45 : 1,
      transition: "all 0.15s ease", ...sizes[size], ...variants[variant], ...style,
    }}>{children}</button>
  );
};

interface CardProps { children: ReactNode; style?: CSSProperties; glow?: "accent" | "cyan" | "gold"; onClick?: () => void; className?: string; }
export const Card: React.FC<CardProps> = ({ children, style = {}, glow, onClick, className }) => {
  ...
  return (
    <div className={className} onClick={onClick} style={{
      background: C.card, border: `1px solid ${C.border}`, borderRadius: 9, padding: 22,
      ...(glow ? glowMap[glow] : {}),
...
export const Logo: React.FC<{ size?: number }> = ({ size = 32 }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
    <div style={{
      ...
      boxShadow: `0 0 18px ${C.accentGlow}`, flexShrink: 0,
    }}>⚡</div>
    <span style={{
...
export const Timer: React.FC<{ seconds: number; max: number }> = ({ seconds, max }) => {
  ...
  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontFamily: FONT_DISPLAY, fontSize: 46, fontWeight: 900, color, textShadow: `0 0 16px ${color}`, lineHeight: 1, animation: seconds <= 5 ? "pulseGlow 0.5s ease-in-out infinite" : "none" }}>{seconds}</div>
      <div style={{ height: 4, background: C.border, borderRadius: 2, marginTop: 7, overflow: "hidden", width: 80 }}>
        <div style={{ height: "100%", width: `${pct}%`, background: color, transition: "width 1s linear", borderRadius: 2 }} />
      </div>
    </div>
  );
};
...
export const Badge: React.FC<BadgeProps> = ({ children, color = C.muted }) => (
  <span style={{ display: "inline-block", padding: "3px 10px", background: `${color}22`, border: `1px solid ${color}55`, borderRadius: 20, fontSize: 12, color, fontFamily: FONT_DISPLAY, letterSpacing: "0.08em" }}>{children}</span>
);

interface FieldProps { label?: string; hint?: string; children: ReactNode; required?: boolean; }
export const Field: React.FC<FieldProps> = ({ label, hint, children, required }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
    {label && <label style={{ fontSize: 12, color: C.muted, letterSpacing: "0.12em", textTransform: "uppercase", display: "flex", gap: 4 }}>{label}{required && <span style={{ color: C.accent }}>*</span>}</label>}
    {children}
...
export const BackBtn: React.FC<{ onClick: () => void; label?: string }> = ({ onClick, label = "Back" }) => (
  <button onClick={onClick} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", gap: 6, fontFamily: FONT_BODY, letterSpacing: "0.05em", marginBottom: 26, padding: 0 }}>← {label}</button>
);
...
export const LiveDot: React.FC<{ count?: number }> = ({ count }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
    ...
    {count !== undefined && <span style={{ fontSize: 13, color: C.success, fontFamily: FONT_DISPLAY, letterSpacing: "0.05em" }}>{count} LIVE</span>}
  </div>
);
...
export const QTypeIcon: React.FC<{ type: string }> = ({ type }) => {
  ...
  return (
    <span title={info.label} style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: 5, background: `${info.color}18`, border: `1px solid ${info.color}44`, color: info.color, fontSize: 14, fontWeight: 700 }}>{info.icon}</span>
  );
};
