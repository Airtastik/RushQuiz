...
import { generateRoomCode, downloadCsv, csvToQuiz, uid } from "../utils";
import { Btn, Card, Logo, BackBtn, Field, Badge, QTypeIcon, Empty, Divider } from "../components/ui";
...
export const HomeScreen: React.FC<HomeScreenProps> = ({ onHost, onJoin }) => (
  <div className="grid-bg" style={{
    ...
    <div className="fade-up" style={{ textAlign: "center", maxWidth: 580, zIndex: 1 }}>
      ...
      <div style={{ fontFamily: FONT_DISPLAY, fontSize: 13, letterSpacing: "0.3em", color: C.muted, marginBottom: 10 }}>
      ...
      <p style={{ color: C.muted, fontSize: 17, lineHeight: 1.65, marginBottom: 48 }}>
        Host a live quiz for 50+ players or jump into a game with just a room code.
        Create multi-type quizzes and export them as CSV.
      </p>

      <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
        <Btn size="lg" onClick={onHost} style={{ minWidth: 200 }}>🎮 Host a Game</Btn>
        <Btn size="lg" variant="ghost" onClick={onJoin} style={{ minWidth: 200, borderColor: C.cyan, color: C.cyan }}>
          🚀 Join Game
        </Btn>
      </div>
      ...
      <div style={{ display: "flex", gap: 40, justifyContent: "center", marginTop: 56, flexWrap: "wrap" }}>
        {[
          { num: "50+",    label: "Players / Room" },
          { num: "3",      label: "Question Types" },
...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onBack, onAuth }) => {
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, background: C.bg }} className="grid-bg">
      <div className="fade-up" style={{ width: "100%", maxWidth: 420 }}>
        ...
        <Card>
          ...
          <div style={{ display: "flex", gap: 0, marginBottom: 22, border: `1px solid ${C.border}`, borderRadius: 6, overflow: "hidden" }}>
            {(["login", "register"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} style={{
                flex: 1, padding: "10px", background: tab === t ? C.accent : "transparent",
                border: "none", color: tab === t ? "#fff" : C.muted,
                fontFamily: FONT_BODY, fontWeight: 700, fontSize: 14,
                textTransform: "uppercase", letterSpacing: "0.08em", cursor: "pointer", transition: "all 0.2s",
              }}>{t}</button>
            ))}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {tab === "register" && (
              <Field label="Display Name" required>
                <input style={inputStyle} placeholder="Your quiz host name" value={name} onChange={e => setName(e.target.value)} />
              </Field>
            )}
...
interface JoinScreenProps {
  onBack: () => void;
  onJoin: (info: { code: string; playerName: string }) => void;
}

export const JoinScreen: React.FC<JoinScreenProps> = ({ onBack, onJoin }) => {
        ...
        <Card>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <Field label="Your Name" required>
              <input
                style={{ width: "100%", padding: "11px 14px", borderRadius: 6, background: C.surface, border: `1px solid ${C.border}`, color: C.text, fontFamily: FONT_BODY, fontSize: 15 }}
            ...
            <Field label="Room Code" required>
              <input
                style={{ width: "100%", padding: "12px 14px", borderRadius: 6, background: C.surface, border: `1px solid ${C.accent}`, color: C.accent, fontFamily: FONT_DISPLAY, fontSize: 24, textAlign: "center", letterSpacing: "0.22em", textTransform: "uppercase" as const }}
                placeholder="ABC123"
                value={code}
            ...
            <Btn
              onClick={() => onJoin({ code, playerName: playerName || "Player" })}
              disabled={code.length < 4 || !playerName.trim()}
              style={{ width: "100%" }}
            >
              🚀 Join Room
            </Btn>
...
export const Dashboard: React.FC<DashboardProps> = ({
  user, quizzes, onCreateQuiz, onEditQuiz, onDeleteQuiz, onStartGame, onImportQuiz, onBack,
}) => {
      ...
      <div style={{ maxWidth: 980, margin: "0 auto" }}>
        ...
        <Card glow="accent" style={{ marginBottom: 30, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
          <div>
            ...
            <div className="neon-red" style={{ fontFamily: FONT_DISPLAY, fontSize: 42, fontWeight: 900, color: C.accent, letterSpacing: "0.22em", lineHeight: 1 }}>
              {roomCode}
            </div>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {selectedQuiz && (
              <Btn size="md" onClick={() => onStartGame(selectedQuiz, roomCode)}>
                ▶ Launch Game
              </Btn>
            )}
          </div>
        ...
        {!selectedQuiz && quizzes.length > 0 && (
          <p style={{ textAlign: "center", color: C.muted, marginTop: 22, fontSize: 14 }}>
            Select a quiz above, then click Launch Game
          </p>
        )}
      </div>
    </div>
...
const QuizCard: React.FC<QuizCardProps> = ({ quiz, selected, onSelect, onEdit, onDelete, onExport }) => {
  const typeCounts = {
    mc: quiz.questions.filter(q => q.type === "multiple_choice").length,
    sa: quiz.questions.filter(q => q.type === "short_answer").length,
    nu: quiz.questions.filter(q => q.type === "numeric").length,
  };
  ...
  return (
    <div
      className="fade-up"
      style={{
        background: selected ? `${C.accent}10` : C.card,
    ...
    >
      <div onClick={onSelect} style={{ padding: "18px 18px 14px" }}>
        <div style={{ fontFamily: FONT_DISPLAY, fontSize: 14, marginBottom: 6, lineHeight: 1.3 }}>{quiz.title}</div>
        {quiz.description && <div style={{ fontSize: 13, color: C.muted, marginBottom: 10 }}>{quiz.description}</div>}

        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
          {typeCounts.mc > 0 && <Badge color={C.cyan}>{typeCounts.mc} MC</Badge>}
          {typeCounts.sa > 0 && <Badge color={C.warning}>{typeCounts.sa} SA</Badge>}
          {typeCounts.nu > 0 && <Badge color={C.success}>{typeCounts.nu} Num</Badge>}
        </div>

        <div style={{ fontSize: 12, color: C.muted }}>
          {quiz.questions.length} questions · {quiz.questions.reduce((s, q) => s + q.points, 0)} pts
        </div>

        {selected && (
          <div style={{ marginTop: 12, padding: "7px 12px", background: C.accent, borderRadius: 5, fontSize: 12, fontWeight: 700, textAlign: "center", letterSpacing: "0.1em" }}>
            SELECTED ✓
          </div>
        )}
      </div>
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, display: "flex" }}>
        {[
          { label: "Edit",   fn: onEdit,   col: C.cyan },
          { label: "CSV",    fn: onExport, col: C.success },
          { label: "Delete", fn: onDelete, col: C.error },
        ].map(({ label, fn, col }) => (
          <button key={label} onClick={e => { e.stopPropagation(); fn(); }} style={{
            flex: 1, padding: "8px", background: "none", border: "none",
            borderRight: label !== "Delete" ? `1px solid ${C.border}` : "none",
            color: col, fontSize: 12, fontFamily: FONT_BODY, fontWeight: 700,
